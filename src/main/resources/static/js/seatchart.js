...
function SeatchartJS(seatMap, seatTypes) { // eslint-disable-line no-unused-vars
    ...
    */
    ...
    var updateShoppingCart = function updateShoppingCart(action, id, type, previousType) {
        ...
        var scItem;
        if (type == 'road') {
            var capitalizedType = "通道";
        } else {
            var capitalizedType = "维修中";
        }
    ...
    this.createLegend = function createLegend(containerId) {
        ...
        var legendTitle = createTitle('图例：');

        var seatsListTitle = createSmallTitle(' ');
        var seatsList = createLegendList();
        seatsList.appendChild(createLegendItem('可用', 'available'));
        seatsList.appendChild(createLegendItem('已锁定', 'unavailable'));

        var yourSeatsListTitle = createSmallTitle(' ');
        ...
        // for (var i = 0; i < seatTypes.length; i += 1) {
            ...
            var item = createLegendItem(description, '', seatTypes[0].color);
            yourSeatsList.appendChild(item);

            var description1 = '{0} {1}{2}'.format(
                "维修中",
                " ",
                self.currency
            );
            var item1 = createLegendItem(description1, '', seatTypes[1].color);
            yourSeatsList.appendChild(item1);
        // }
